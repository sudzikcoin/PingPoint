...
<body>
  ...
  // GET /api/internal/loads/:loadNumber/trip-stats
  // Aggregates this load's tracking_pings into a TripStats object on demand.
  // No DB schema change — computation is pure and happens here, at request
  // time (typically once, right after delivery, when AgentOS computes CO2).
  // Auth: same x-internal-key as the other /api/internal/loads/* routes.
  app.get("/api/internal/loads/:loadNumber/trip-stats", async (req: Request, res: Response) => {
    try {
      ...
      const pings = await storage.getTrackingPingsByLoad(load.id);
      // getTrackingPingsByLoad returns DESC by created_at; computeTripStats
      // sorts internally, but passing ASC-ish keeps debugging sane.
      pings.reverse();
      const { computeTripStats } = await import("./pingAnalytics");
    ...
    try {
      ...
      const safeResponse = {
        ...
        lastLocation: latestPing ? {
          ...
          // Mobile app reports speed in m/s (Android Location.getSpeed default).
          // Convert to mph here so the client can render it directly without
          // needing to know the unit convention.
          speedMph: latestPing.speed != null && Number.isFinite(Number(latestPing.speed))
            ? Number((Number(latestPing.speed) * 2.23693629).toFixed(1))
            : null,
        } : null,
      };
  ...
  app.get("/api/driver/:token/next-load", async (req: Request, res: Response) => {
    try {
      ...
      // Ищем следующий груз: если есть driverId — по нему, иначе по driverToken prefix
      let nextLoad = null;
      if (currentLoad.driverId) {
        nextLoad = await storage.getNextLoadForDriver(currentLoad.id, currentLoad.driverId);
      }
      // Fallback: ищем по broker + похожему driver_token (тот же водитель)
      if (!nextLoad) {
        nextLoad = await storage.getNextLoadByBrokerAndToken(
          currentLoad.id,
          currentLoad.driverToken,
          currentLoad.brokerId
        );
      }
      if (nextLoad) {
        return res.json({
  ...
  // 422 responses do not consume the bucket — a brief spell of bad GPS shouldn't lock the driver out.
  app.post("/api/driver/:token/ping", strictRateLimit(60, 60000, [422]), async (req: Request, res: Response) => {
    try {
      const { token } = req.params;
      ...
      if (lastPing && lastPing.lat && lastPing.lng) {
        ...
        // Reconnect-after-gap: if too much time has passed since the last ping,
        // we can't infer actual speed (the driver may have been moving the
        // whole time and we just missed the intermediate samples). Accept the
        // new ping as a fresh baseline, flagged low-confidence. Without this,
        // a single stale stored ping would make every subsequent ping look
        // like a 200+ km teleport.
        const RECONNECT_GAP_MS = 15 * 60 * 1000;
        if (timeDeltaMs > RECONNECT_GAP_MS) {
          lowConfidence = true;
          console.log(`[TrackingPing] reconnect-accept (gap too large for speed inference): gap=${Math.round(timeDeltaMs / 1000)}s dist=${distanceM.toFixed(0)}m`);
          ...
          if (speedMph > GPS_MAX_SPEED_MPH) {
            // Distinguish real teleportation from GPS jitter: when the combined
            // accuracy uncertainty of the two fixes can account for the apparent
            // move, it's sensor noise (common for stationary vehicles with poor
            // sky view), not a teleport. Accept but flag as low-confidence —
            // the `accuracy` column on the stored ping already carries the signal.
            const prevAcc = lastPing.accuracy ? parseFloat(lastPing.accuracy) : 0;
            const curAcc = accuracy != null ? parseFloat(accuracy.toString()) : 0;
            const combinedAccuracy = prevAcc + curAcc;
            if (combinedAccuracy > 0 && distanceM < combinedAccuracy) {
              lowConfidence = true;
              console.log(`[TrackingPing] low-confidence accept (jitter within combined accuracy): dist=${distanceM.toFixed(0)}m combinedAcc=${combinedAccuracy.toFixed(0)}m speed=${speedMph.toFixed(1)}mph`);
            } else {
              console.log(`[TrackingPing] REJECTED reason=teleport_detected speed=${speedMph.toFixed(1)}mph dist=${distanceM.toFixed(0)}m delta=${timeDeltaMs}ms prevAcc=${prevAcc}m curAcc=${curAcc}m`);
  ...
  // 422 responses do not consume the bucket (see /ping route comment).
  app.post("/api/driver/location", strictRateLimit(60, 60000, [422]), async (req: Request, res: Response) => {
    try {
      ...
      if (lastPing && lastPing.lat && lastPing.lng) {
        ...
        // Reconnect-after-gap: see /api/driver/:token/ping for rationale.
        const RECONNECT_GAP_MS = 15 * 60 * 1000;
        if (timeDeltaMs > RECONNECT_GAP_MS) {
          lowConfidence = true;
          console.log(`[TrackingPing] reconnect-accept (gap too large for speed inference): gap=${Math.round(timeDeltaMs / 1000)}s dist=${distanceM.toFixed(0)}m`);
          ...
          const speedMph = calculateSpeedMph(distanceM, timeDeltaMs);
          if (speedMph > GPS_MAX_SPEED_MPH) {
            // Accuracy-aware jitter check — see /api/driver/:token/ping for rationale.
            const prevAcc = lastPing.accuracy ? parseFloat(lastPing.accuracy) : 0;
            const curAcc = accuracy != null ? parseFloat(accuracy.toString()) : 0;
            const combinedAccuracy = prevAcc + curAcc;
            if (combinedAccuracy > 0 && distanceM < combinedAccuracy) {
              ...
              console.log(`[TrackingPing] REJECTED reason=teleport_detected speed=${speedMph.toFixed(1)}mph dist=${distanceM.toFixed(0)}m prevAcc=${prevAcc}m curAcc=${curAcc}m`);
              return res.status(422).json({ ok: false, error: "Unrealistic GPS jump" });
            }
          }
        }
      ...
      return res.json({ ok: true, pingId: ping.id, lowConfidence: lowConfidence || undefined });
    } catch (error) {
  ...
  app.post("/api/internal/loads", async (req: Request, res: Response) => {
    try {
      ...
      // Dedup: if a load with this customerRef already exists, return it instead of creating a duplicate
      const existingLoad = await storage.getLoadByCustomerRef(customerRef);
      if (existingLoad) {
        console.log(`[InternalAPI] Dedup: load with customerRef=${customerRef} already exists as ${existingLoad.loadNumber} — returning existing`);
        const existingTrackingLink = `${process.env.PUBLIC_BASE_URL || "https://pingpoint.suverse.io"}/track/${existingLoad.trackingToken}`;
        const existingDriverLink = `${process.env.PUBLIC_BASE_URL || "https://pingpoint.suverse.io"}/driver/${existingLoad.driverToken}`;
        return res.status(200).json({
          ok: true,
          loadId: existingLoad.id,
          loadNumber: existingLoad.loadNumber,
          trackingLink: existingTrackingLink,
          driverWebLink: existingDriverLink,
          driverAppLink: existingDriverLink,
          deduplicated: true,
        });
      }
