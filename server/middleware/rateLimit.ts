...
export interface RateLimitOptions {
  ...
  message?: string;
  useUserId?: boolean;
  // Status codes that should NOT consume the bucket. If the final response
  // status matches, the counter is decremented on `res.finish`. Useful so
  // validation rejections (e.g. 422) don't let a client lock itself out.
  skipCountOnStatus?: number[];
}
...
export function strictRateLimit(
  ...
  return rateLimit({ maxRequests, windowMs, skipCountOnStatus });
}
