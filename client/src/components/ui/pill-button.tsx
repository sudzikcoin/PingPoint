...
const PillButton = React.forwardRef<HTMLButtonElement, PillButtonProps>(
  ({ className, variant = "gold", size = "md", icon, children, ...props }, ref) => {
    ...
    const variantStyles = {
      gold: "bg-image-pill-gold text-[#6b3b05] shadow-pill-gold border-none hover:brightness-110",
      dark: "bg-image-pill-dark text-brand-text shadow-pill-dark border border-white/10 hover:brightness-125",
      light: "bg-image-pill-light text-slate-700 shadow-md border border-white/40 hover:brightness-95",
    };
    ...
    return (
      <button
        ref={ref}
        className={cn(
          "inline-flex items-center justify-between rounded-full transition-all select-none cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98]",
